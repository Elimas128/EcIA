#creating a histogram for terrestrial invertebrates found in the southern plot 
...
Splot_data_filtered <- Splot_data %>%
  ...
  slice(0:84)  # Exclude rows 85 to 100 which include aquatic invertebrates 
...
ggplot(Splot_data_filtered, aes(x = factor(class), y = total_organisms, fill = factor(order))) +
  ...
  labs(title = "Terrestrial invertebrate orders found on the Southern plot",
       x = "class",
       y = "Organism Quantity (n)",
       fill = "order") +
  scale_y_continuous(
    labels = scales::comma_format(),
    breaks = seq(0, max(Splot_data_filtered$total_organisms, na.rm = TRUE), by = 5),
    name = "Individuals",
    expand = c(0, 0) 
  ) +
...
Nplot_data_filtered <- Nplot_data %>%
  filter(phylum %in% c("Arthropoda", "Mollusca", "Annelida"), !is.na(organismQuantity)) %>%
  slice(0:84)  # Exclude rows 85 to 100 which include aquatic invertebrates 
...
ggplot(Nplot_data_filtered, aes(x = factor(class), y = total_organisms, fill = factor(order))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Terrestrial invertebrate orders found on the Northern plot",
       x = "class",
       y = "Organism Quantity (n)",
       fill = "order") +
  scale_y_continuous(
    labels = scales::comma_format(),
    breaks = seq(0, max(Nplot_data_filtered$total_organisms, na.rm = TRUE), by = 5),
    name = "Individuals",
    expand = c(0, 0) 
  ) +
